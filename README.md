# 로컬 이미지 생성·편집 스튜디오 (gpt-image-2 / ChatGPT OAuth)

> 저장소: `codex-image-studio`
> **자연어로 이미지를 생성·수정·병합하는 로컬 웹앱 — API 키 없이 ChatGPT 계정 OAuth로 gpt-image-2 사용**
...
## 다른 PC에 설치 (더블클릭만으로)
이 앱은 100% 로컬 실행이라 PC마다 따로 설치한다. **PC마다 ChatGPT 로그인이 1회 필요**하다
(토큰은 각 PC의 `~/.codex/auth.json` 에 저장됨 — 로컬 프로그램의 필수 구조).

미리 한 번만 설치(각각 더블클릭 설치 파일):
- **Python 3.10+** — https://www.python.org/downloads/ (설치 시 "Add Python to PATH" 체크)
- **Node.js** — https://nodejs.org/ (codex CLI 설치에 필요)

그다음:
1. GitHub 저장소 페이지에서 **Code ▸ Download ZIP** → 압축 풀기 (git 불필요).
2. **`setup.bat` 더블클릭** — 의존성 설치 + codex CLI 자동 설치 + (미로그인 시) 브라우저로
   ChatGPT 로그인 자동 실행.
3. **`run.bat` 더블클릭** — 브라우저가 자동으로 열린다.

※ 같은 ChatGPT 계정으로 여러 PC에 로그인하면 이미지 생성 할당량은 계정 단위로 공유(합산)된다.
